...
export default function MX1({ FX1, controlsRef, CJR1 }: { FX1: THREE.Vector3, controlsRef: any, CJR1: any }) {
  ...
  const DM1 = useRef<THREE.Object3D[]>([])
  ...
  useFrame((state, delta) => {
    ...
    if (CJR1.current && DM1.current.length === 0) {
    ...
    if (FX1.length() > 0) {
      ...
      let PZ1 = false
      if (DM1.current.length > 0) {
        const YC1 = new THREE.Raycaster(XR1.current.position.clone().add(new THREE.Vector3(0, 0.5, 0)), dir.clone().normalize(), 0, 0.6)
        const JZ1 = YC1.intersectObjects(DM1.current, false)
        if (JZ1.length > 0) PZ1 = true
      }

      const NX1 = XR1.current.position.x + step.x
      const NZ1 = XR1.current.position.z + step.z

      if (!PZ1 && Math.abs(NX1) < 22 && Math.abs(NZ1) < 22) {
    ...
    if (DM1.current.length > 0) {
      ...
      const JZ2 = YC2.intersectObjects(DM1.current, false)
      if (JZ2.length > 0) {
